...
namespace dft {
	...
	static void DFTInit(int n0, int nf, int* factors, int* itab, Complex<float>* _wave);
	static void fft(const Complex<float> *src, Complex<float> *dst, int n, int nf, const int *factors, const int *itab, Complex<float> *wave, int inv = 0);
...
#define DFT_PI 3.1415926535897932384626433832795
...
#define BitRev(i,shift) \
   ((int)((((unsigned)bitrevTab[(i)&255] << 24)+ \
	...
	{
		int l = 0, r = sizeof(optimalDFTSizeTab) / sizeof(optimalDFTSizeTab[0]) - 1;
		if ((unsigned)n >= (unsigned)optimalDFTSizeTab[r])
			return -1;
		
		while (l < r) {
			int mid = (l + r) >> 1;
			if (n <= optimalDFTSizeTab[mid])
				r = mid;
			else
				l = mid + 1;
		}
		return optimalDFTSizeTab[l];
	}
	...
	static void fft(const Complex<float> *src, Complex<float> *dst, int n, int nf, const int *factors, const int *itab, Complex<float> *wave, int inv)
	{
		...
		// shuffle data
		if (!inv) {
			for (int i = 0; i <= n - 2; i += 2, itab += 2) {
				int k0 = itab[0], k1 = itab[1];
	...
	static void DFTInit(int n0, int nf, int* factors, int* itab, Complex<float>* _wave)
	{
		...
		if (n0 <= 5) {
			...
			for (int i = 1; i < n; i++) {
				_wave[i].re = (float)w.re;
				_wave[i].im = (float)w.im;
				_wave[n0 - i].re = (float)w.re;
				_wave[n0 - i].im = (float)-w.im;
